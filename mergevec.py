...
@author: Ian
	...
	To use the function:
	(1) Place all .vec files to be merged in a single directory (opencv_workspace)
	(2) Navigate to this file in your CLI (terminal or cmd) and type "python mergevec.py -v opencv_workspace -o combined_positives.vec".
	...
	(2) Type "opencv_createsamples -w 50 -h 50 -vec combined_positives.vec"
...
def get_args():
	...
	parser.add_argument('-v', dest='vec_directory')      #argparse treats the options given as strings, unless we tell it otherwise
...
def merge_vec_files(vec_directory, output_vec_file):
	...
	try:
		...
		print('An IO error occured while processing the file: {0}'.format(e))
